##START,aws_kms_key
resource "aws_kms_key" "k-2d0349ec-c2ea-44b4-8839-105cd1994e02" {
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Effect = "Allow"
      Principal = {
        AWS = [format("arn:aws:iam::%s:root",data.aws_caller_identity.current.account_id), format("arn:aws:sts::%s:assumed-role/WSParticipantRole/Participant",data.aws_caller_identity.current.account_id)]
      }
      Resource = "*"
